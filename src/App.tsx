...
export default function App() {
  ...
  const handleUpload = () => {
    if (resume.trim() && jd.trim()) {
      // Simple parsing: split resume into paragraphs/blocks
      ...
      <header className="border-b border-slate-200/50 bg-white/80 backdrop-blur-xl sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-semibold text-[17px] tracking-tight text-slate-900">AI Job Assistant</h1>
          </div>
          ...
          {step === 'parse' && (
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-[32px] font-semibold tracking-tight text-slate-900">内容确认</h2>
                  <p className="text-slate-500 mt-2 font-light">请确认解析出的简历内容，并选择一个分析维度。</p>
                </div>
                <button onClick={reset} className="text-slate-400 hover:text-slate-800 transition-colors flex items-center gap-1.5 text-[14px] font-medium bg-white px-4 py-2 rounded-full border border-slate-200 shadow-sm">
              ...
              <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6">
                  <div className="bg-white rounded-[32px] border border-slate-200/60 p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                    <h3 className="text-[13px] font-medium uppercase tracking-widest text-slate-400 mb-8">简历内容块 (选中文字可进行针对性改写)</h3>
                    <div 
                      className="space-y-4 max-h-[600px] overflow-y-auto pr-6 custom-scrollbar"
                      onMouseUp={handleTextSelection}
                    >
                      {resumeBlocks.length > 0 ? resumeBlocks.map((block, i) => (
                        <div key={i} className="p-6 rounded-[20px] bg-slate-50/50 border border-slate-100 text-slate-700 leading-[1.8] text-[15px] hover:bg-white hover:border-slate-300 hover:shadow-sm transition-all cursor-text">
                          {block}
                        </div>
                      )) : (
                ...
                <div className="space-y-6">
                  <div className="bg-slate-900 text-white rounded-[32px] p-10 shadow-[0_20px_40px_rgb(15,23,42,0.2)] sticky top-24">
                    <h3 className="text-[13px] font-medium uppercase tracking-widest text-slate-400 mb-8">选择分析维度</h3>
                    <div className="space-y-4">
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full bg-indigo-500/20 text-indigo-300 flex items-center justify-center"><Search size={20} /></div>
                          <div className="text-left">
                            <div className="font-medium text-[15px]">匹配度分析</div>
                            <div className="text-[13px] text-slate-400 mt-0.5">亮点挖掘与能力缺口</div>
                          </div>
                        </div>
                      ...
                      <button 
                        ...
                        className={`w-full flex items-center justify-between p-5 rounded-[20px] transition-all group ${selectedText ? 'bg-white/5 hover:bg-white/10 border border-white/5' : 'opacity-40 cursor-not-allowed border border-dashed border-white/10'}`}
                      >
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full bg-emerald-500/20 text-emerald-300 flex items-center justify-center"><Edit3 size={20} /></div>
                          <div className="text-left">
                            <div className="font-medium text-[15px]">简历改写建议</div>
                            <div className="text-[13px] text-slate-400 mt-0.5">{selectedText ? '已选中片段' : '请先在左侧选中文字'}</div>
                          </div>
                        </div>
                        <ChevronRight size={18} className="text-slate-500 group-hover:translate-x-1 transition-transform" />
                      </button>
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full bg-amber-500/20 text-amber-300 flex items-center justify-center"><MessageSquare size={20} /></div>
                          <div className="text-left">
                            <div className="font-medium text-[15px]">面试问题预测</div>
                            <div className="text-[13px] text-slate-400 mt-0.5">问题预测与意图解析</div>
                          </div>
                        </div>
                        <ChevronRight size={18} className="text-slate-500 group-hover:translate-x-1 transition-transform" />
                      </button>
                    ...
                    {selectedText && (
                      ...
                      >
                        <div className="text-[11px] font-medium text-slate-400 uppercase tracking-widest mb-3">已选中待改写片段</div>
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                  <button onClick={() => setStep('parse')} className="w-12 h-12 rounded-full bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-colors shadow-sm text-slate-600">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 mt-1 font-light">基于您的简历与 JD 深度生成的结构化建议。</p>
                  </div>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => runTask(currentTask!)} className="flex items-center gap-2 bg-white border border-slate-200 px-5 py-2.5 rounded-full text-[14px] font-medium text-slate-700 hover:bg-slate-50 transition-all shadow-sm">
                    <RotateCcw size={16} /> 重新生成
                  </button>
                  <button onClick={reset} className="flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-full text-[14px] font-medium hover:bg-slate-800 transition-all shadow-md">
                    开始新分析
                  </button>
                </div>
              </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                      ...
                      <div>
                        ...
                        <ul className="space-y-4">
                          {result.highlights.map((h: string, i: number) => (
                            <li key={i} className="flex gap-4 text-[15px] text-slate-700 leading-[1.7] font-light">
                    ...
                    <div className="space-y-8">
                      <div className="bg-slate-50 rounded-[32px] p-10 border border-slate-100">
                        ...
                        <p className="text-slate-600 leading-[1.8] text-[15px] font-light">"{result.summary}"</p>
                      </div>
                      <div className="bg-white rounded-[32px] border border-slate-200/60 p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                        ...
                        <div className="space-y-3">
                          {result.evidence.map((e: string, i: number) => (
                            <div key={i} className="text-[13px] text-slate-500 bg-slate-50/50 p-4 rounded-[16px] border border-slate-100 font-light leading-relaxed">
                              {e}
                            </div>
                          ))}
                        </div>
    ...
    <div className="bg-white rounded-[28px] border border-slate-200/60 p-8 shadow-sm">
                ...
                {currentTask === 'interview' && result && (
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {result.questions.map((q: any, i: number) => (
                      <div key={i} className="bg-white rounded-[32px] border border-slate-200/60 p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col h-full">
                        <div className="mb-6">
                          <span className="text-[11px] font-medium uppercase tracking-widest text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-full">问题 {i + 1}</span>
                        </div>
                        <h3 className="font-medium text-[17px] text-slate-900 mb-6 leading-[1.6]">{q.question}</h3>
                        <div className="mt-auto space-y-5">
                          <div className="pt-6 border-t border-slate-100">
                            <div className="text-[11px] font-medium text-slate-400 uppercase tracking-widest mb-2">面试官意图</div>
                            <p className="text-[13px] text-slate-600 leading-relaxed font-light">{q.intent}</p>
                          </div>
                          <div className="p-4 rounded-[16px] bg-slate-50/50 border border-slate-100">
                            <div className="text-[11px] font-medium text-slate-400 uppercase tracking-widest mb-2">追问路径</div>
                            <p className="text-[13px] text-slate-700 leading-relaxed font-light">"{q.followUp}"</p>
                          </div>
                        </div>
                      </div>
                    ))}
      ...
      {loading && (
        <div className="fixed inset-0 bg-white/60 backdrop-blur-md z-50 flex flex-col items-center justify-center">
          ...
          <p className="font-medium text-[17px] text-slate-900 tracking-tight">系统正在深度解析您的数据...</p>
          <p className="text-[14px] text-slate-500 mt-2 font-light">正在构建任务驱动的上下文</p>
        </div>
      )}
